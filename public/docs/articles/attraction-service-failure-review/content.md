...
첫 번째 문제는 앞에서도 다루었지만, `TailWindCSS`와 관련 라이브러리 이해도가 부족했었던 것이었습니다.
...
#### 2-3-2. 적절한 라이브러리 사용의 부재

당시 제가 프로덕트의 모든 디자인을 담당하고 있었는데 항상 마크업과 스타일링 파트에서 문제가 있어 검수를 하기 십상이었습니다. 그래서 저는 기존 `Shadcn/UI`로 구성했던 기존의 디자인 시스템을 점진적으로 저희만의 온전한 커스텀 디자인 시스템으로 바꾸는 작업을 하게 되었습니다. 게다가 위의 `prefix` 차이로 인한 기본 `preflight` 스타일이 중복되는 문제도 있었어서 `Mantine` 라이브러리를 참고하여 `SCSS` 기반으로 다시 작성하는 작업을 하게 되었습니다.

(인프런 블로그에서 봤던 내용이 영향을 주긴 했었습니다.)

<br />

두 번째 문제가 여기서 나오는데, 디자인 시스템 구축을 너무 쉽게 생각했다는 것입니다.
...
저희팀은 코드의 작성 방식에 대한 컨벤션을 수립하지 않았었습니다.
