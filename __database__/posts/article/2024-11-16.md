...
그렇기에 선택지가 `React`로 좁혀졌습니다. `React`는 메타 프레임워크 없이 그냥 사용한다면 `CSR`방식으로 동작하게 됩니다.
...
`Next.js`에서는 `Static Route`에 자동으로 `SSG` 빌드를 제공해주기도 하고 숙련도 또한 조금 있었던 상태였기 때문에 다시 돌아간다면 `Next.js`를 사용했을 것 같습니다.
...
개인적인 경험으로 `module CSS` 나 `SCSS` 같은 경우에는 스크립트를 함께 사용하기에 조금 아쉬운 느낌이 없지 않아 있었고,

`Styled Components`의 경우에는 `SSR` 환경을 지원하지 않았습니다.

<br />

또한 당시에는 좋지 않은 방법으로 `TailWindCSS`를 사용했었기에 `TailWindCSS`의 사용 경험이 좋지 않았던 상태였어서, 익숙했던 `SCSS` 문법과 `SSR` 환경을 지원하는 `Emotion`으로 선택하였습니다.
...
(시도해보았지만 역시나 `@emotion/styled` 패키지는 `SSR`이 적용이 되지 않았습니다.)
...
![SSG 빌드 결과 1](</img/content/2024-11-16 (1).jpg>)

<br />

![SSG 빌드 결과 2](</img/content/2024-11-16 (2).jpg>)
